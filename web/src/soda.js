...
//   id           (string)        — parcel record id
//   parcel_owner (text)          — usually "PPD"
//   zoning       (text)          — zoning code at the parcel
//   area         (number)        — square metres
//   location     (Polygon)       — WGS84 polygon
 ...
 * feature is a polygon with parcel_owner ("PPD" for the bulk of
 * the dataset), zoning code, and area in square metres. Memoised
 * via the shared fetchAllAndCache helper so subsequent toggles
 * read from in-memory cache without re-hitting Socrata.
 */
export async function fetchCityOwnedParcels() {
  return fetchAllAndCache('cityOwnedParcels', CITY_OWNED_PARCELS_URL);
}

// Static GeoJSON files generated by web/scripts/build-transit-geojson.mjs
// from the Winnipeg Transit GTFS feed. Re-run that script (`npm run
// refresh:transit`) to refresh the overlays; the public/*.geojson
// files are committed so deploys work without rebuilding.
...
/**
 * Memoised fetch for the static GeoJSON files we ship under
 * web/public/. Mirrors the in-flight-promise pattern used by
 * fetchAllAndCache so a rapid double-toggle doesn't double-fetch.
 */
async function fetchStaticGeoJson(key, url) {
  if (STATIC_GEOJSON_CACHE.has(key)) return STATIC_GEOJSON_CACHE.get(key);
  const promise = (async () => {
    const res = await fetch(url);
    if (!res.ok) throw new Error(`HTTP ${res.status} fetching ${url}`);
    return res.json();
  })();
  STATIC_GEOJSON_CACHE.set(key, promise);
  try {
    return await promise;
  } catch (err) {
    STATIC_GEOJSON_CACHE.delete(key);
    throw err;
  }
}

/**
 * Fetch the Winnipeg Transit route-line overlay. Each feature is
 * a MultiLineString stamped with the agency's official route_color
 * (with white/cream locals swapped for a visible grey by the build
 * script). Refresh cadence: monthly via `npm run refresh:transit`.
 */
export async function fetchTransitRoutes() {
  return fetchStaticGeoJson('transitRoutes', TRANSIT_ROUTES_URL);
}

/**
 * Fetch the Winnipeg Transit stop-point overlay. Each feature is
 * a Point with stop_code, stop_name, and a comma-separated list
 * of route short names that serve the stop.
 */
