...
/**
 * A PRE-JOIN range test that DEFERS rather than excludes.
 *
 * The post-join twins of these filters read the export's value when it has
 * one and the live record's otherwise, so a row with nothing in the export
 * may still be classified after the fetch. Excluding it here would make the
 * optimisation change the answer. So: null value = pass, and let the real
 * check decide once the record is in hand.
 *
 * The inverse of the "missing is excluded" rule everywhere else in this
 * file, and deliberately so — the same inversion, and for the same reason,
 * as passesPreJoinVacantFilter. A pre-fetch cut is allowed to be a
 * performance shortcut; it is never allowed to be a filter of its own.
 *
 * @param {number|null} value the export's own figure, or null when it has
 *   none. Callers normalize (a year of 0 is not a year).
 */
export function preJoinRangePasses(value, lo, hi) {
  if (lo == null && hi == null) return true;
...
/** The export's own year built for a SaleRecord, or null. The OLDEST year
 *  of a multi-section sale, matching _saleYearBuiltNumeric exactly. */
export function csvYearBuilt(sale) {
  const n = Number(sale?.yearBuiltNumeric);
  return Number.isFinite(n) && n >= 1700 && n <= 2200 ? n : null;
}

/** The export's own building floor area for a SaleRecord, or null —
 *  matching what buildSaleFeatures stamps as _saleLivingArea. */
export function csvBuildingSf(sale) {
  const n = Number(sale?.livingArea);
