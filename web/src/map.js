...
// map is one .pmtiles archive on the same R2 bucket as the ortho years,
// styled in the browser by @protomaps/basemaps. (CARTO's raster tiles,
// used until 2026-09, started watermarking unkeyed requests and are being
// retired.)
//
...
// `pmtiles extract --bbox` and uploaded to the wpg-ortho R2 bucket next
// to the aerial years. Override for local work (e.g. the Protomaps demo
// planet archive) with VITE_BASEMAP_PMTILES_URL in web/.env.local.
const BASEMAP_PMTILES_URL =
  (() => { try { return import.meta.env.VITE_BASEMAP_PMTILES_URL; } catch { return undefined; } })()
  || 'https://pub-f351b204f73e4b2287acad946d79681c.r2.dev/basemap-manitoba.pmtiles';

// The basemap's own layer stack, generated by @protomaps/basemaps for the
// "light" flavor (the closest match to the Positron look the app was
// designed on). Two adjustments:
...
// Every layer starts visible; the basemap menu flips the whole set via
// STREETS_LAYER_IDS.
...
const BASEMAP_STYLE = {
  ...
  layers: [
    // Protomaps streets is the default. Satellite (imagery + the two
