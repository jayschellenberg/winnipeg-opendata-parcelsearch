...
test('the retired rise param is no longer honoured', () => {
  // The Rise FILTER was removed (the Rise column stays), so an old shared
  // link carrying ?rise= must decode to nothing rather than to a key
  // applyUrlState no longer knows how to apply.
  assert.deepEqual(decodeState('?rise=low'), {});
  assert.deepEqual(decodeState('?rise=mid'), {});
});
