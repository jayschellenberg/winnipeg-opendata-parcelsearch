...
14. [Future enhancement: City of Winnipeg ortho-imagery basemap](#14-future-enhancement-city-of-winnipeg-ortho-imagery-basemap)
...
## 14. Future enhancement: City of Winnipeg ortho-imagery basemap

The Esri World Imagery hybrid stack we ship today tops out at roughly 30 cm resolution in Winnipeg. The City publishes their own annual ortho-imagery at **7.5 cm** resolution as downloadable raster files — about 4× sharper, perfect for parcel-level visual analysis (parking stalls, fence lines, HVAC units, yard condition). It's free under the Canada Open Government Licence.

### What's published

Indexed at [Orthographic Photography Index for Winnipeg (`sf35-zz6g`)](https://data.winnipeg.ca/Organizational-Support-Services/Orthographic-Photography-Index-for-Winnipeg/sf35-zz6g). Single-file city-wide mosaics, one per acquisition year:

| Year | Resolution | Size | Direct URL pattern |
|---|---|---|---|
| 2024 | 7.5 cm | 14.1 GB | `wpgopendata.blob.core.windows.net/ortho-photos-2024/MBCWPG24_Property.ecw.zip` |
| 2021 | 7.5 cm | 16.1 GB | `…/ortho-photos-2021/MBCWPG21_Property_Delivery.ecw` |
| 2018 | 7.5 cm | 13.5 GB | `…/ortho-photos-2018/MBCWPG18_Delivery_Updated.ecw` |
| 2016 | 7.5 cm | 10.6 GB | `…/ortho-photos-2016/MBCWPG16_Delivery.ecw` |
| 2014 | 10 cm | 7.7 GB | `…/ortho-photos-2014/MBCWPG14_Delivery.ecw` |
| 2012 | 10 cm | 8.2 GB | `…/ortho-photos-2012/MBCWPG12-ECW-AREA-WIDE-10CM.ecw` |
| 2009 | 20 cm | 3.0 GB | `…/ortho-photos-2009/2009WinnipegUrban_20cm.ecw` |
| 2005 | 20 cm | 1.3 GB | `…/ortho-photos-2005/2005_CoW_25.ecw` |

- **Format:** ECW (Enhanced Compressed Wavelet — proprietary to Hexagon/ERDAS; GDAL reads it via Hexagon's free public SDK)
- **Projection:** NAD 83 UTM Zone 14 (EPSG:26914)
- **No web tile service / WMS / ArcGIS REST endpoint** — all access is direct file download. The City's own PP&D Property Map uses Pictometry internally, which isn't publicly accessible.

### Why we haven't shipped it yet

For most appraisal-tool use cases, Esri's resolution is sufficient. The City ortho earns its keep only when you're reading building details from imagery (parking-stall counts, yard condition, etc.). It's also non-trivial to integrate.

### Pipeline outline if/when it's time

1. **Download the most recent mosaic** (currently 2024 → 14 GB ECW). Single file, no auth needed — `curl -O https://wpgopendata.blob.core.windows.net/ortho-photos-2024/MBCWPG24_Property.ecw.zip`.

2. **Convert ECW → GeoTIFF** with GDAL + the free Hexagon ECW SDK:
   ```
   gdal_translate MBCWPG24_Property.ecw temp.tif \
     -of GTiff -co COMPRESS=DEFLATE -co TILED=YES -co BIGTIFF=YES
   ```
   Takes ~3 hours on a fast machine; intermediate output ~60–80 GB.

3. **Reproject to Web Mercator** (MapLibre's tile coordinate system):
   ```
   gdalwarp -t_srs EPSG:3857 temp.tif reprojected.tif \
     -multi -wo NUM_THREADS=8 -r bilinear
   ```
   Another ~2–4 hours.

4. **Build a PMTiles archive** (single-file, random-access — same format the parcel layer already uses). Two options:
   - `rio-pmtiles reprojected.tif winnipeg-ortho.pmtiles --zoom-levels 12-19 --workers 8`
   - Or `gdal2tiles.py` to per-tile pyramid → `pmtiles convert` to single archive

   Tune the `--zoom-levels` and JPEG quality to keep the output under ~5–8 GB. Anything larger gets unwieldy to ship.

5. **Host** the PMTiles file. Three viable spots:
   - `web/public/winnipeg-ortho.pmtiles` (Vercel free tier handles single static files in /public, but watch the 100 GB monthly bandwidth limit; each tile request adds up)
   - Cloudflare R2 / S3 with public read (no bandwidth fees on R2; ~$0.015/GB/month storage)
   - A second Vercel deployment dedicated to static assets

6. **Wire as a MapLibre basemap source.** Add to the style's sources:
   ```js
   sources: {
     'wpg-ortho': {
       type: 'raster',
       tiles: ['pmtiles://winnipeg-ortho.pmtiles/{z}/{x}/{y}'],
       tileSize: 256,
       minzoom: 12,
       maxzoom: 19,
       attribution: '© City of Winnipeg',
     },
     // ... existing carto-positron + esri sources
   }
   ```
   Then a `wpg-ortho-raster` layer with `layout.visibility: 'none'`, and extend `BasemapToggleControl` to cycle through THREE basemaps (Streets / Esri Imagery / City Ortho) instead of two.

7. **Add an `npm run refresh:ortho` script** under `web/scripts/build-ortho-pmtiles.mjs` — same pattern as `refresh:transit`. Annual cadence; spring is when the new acquisition typically lands on the open-data portal.

### Trade-offs vs. staying on Esri

| | Stay on Esri | Build City Ortho pipeline |
|---|---|---|
| Resolution at z18+ | ~30 cm | ~7.5 cm |
| Currency | Esri global imagery on 1–3 year cycle | Annual (re-run pipeline each spring) |
| Setup work | None | ~1 day initial |
| Annual maintenance | None | ~half-day pipeline re-run |
| Hosting | $0 | Vercel free tier OK; consider R2 if bandwidth grows |
| Attribution | Esri / Maxar / Earthstar | © City of Winnipeg |
| Format risk | Stable (Esri terms unchanged for years) | Hexagon ECW SDK distribution could change |

### Workaround currently shipped

Until the pipeline lands, the **parcel-summary card** and the **citywide-parcels click popup** each include a "PP&D Map →" link. PP&D's legacy mapping page doesn't accept URL parameters, so the link copies the parcel's civic address to clipboard on click and opens PP&D in a new tab — the user pastes into PP&D's search bar to see the parcel on the City's own high-res imagery without leaving the appraisal workflow.
